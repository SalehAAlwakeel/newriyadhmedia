...
export interface SocialConnection {
  ...
  /** Meta page id for Instagram Graph API. */
  metaPageId?: string;
...
export interface GeneratedLogoEntry {
  id: string;
  url: string;
  style: string;
  vibe?: string;
...
export interface BrandKit {
  ...
  logoHistory?: GeneratedLogoEntry[];
}
...
export interface ContentPreferences {
  ...
  postingWindow?: { startHour: number; endHour: number };
}
...
export type CampaignStatus = "planned" | "active" | "done";
...
export async function updateUser(id: string, patch: Partial<User>): Promise<User | null> {
  ...
  if (patch.subscribedAt !== undefined) data.subscribedAt = patch.subscribedAt ?? null;
...
export async function listSubscribedUsers(): Promise<User[]> {
  const rows = await prisma.user.findMany({ where: { subscribed: true } });
...
export async function listPostsByCampaign(userId: string, campaignName: string): Promise<GeneratedPost[]> {
  const rows = await prisma.post.findMany({ where: { userId, campaignName } });
  return rows.map((r) => toPost(r as PostRow));
}
...
export async function deletePostsByCampaign(userId: string, campaignName: string): Promise<number> {
  const result = await prisma.post.deleteMany({ where: { userId, campaignName } });
  return result.count;
}
...
export async function listCampaigns(userId: string): Promise<Campaign[]> {
  ...
  return rows.map((r) => toCampaign(r as CampaignRow));
}
...
export async function createCampaign(campaign: Campaign): Promise<Campaign> {
  const row = await prisma.campaign.create({
    data: {
      ...
      performanceSummary: campaign.performanceSummary ?? null,
    },
  });
